...
IPV6GROUPS = (
    ...
    r'fe80:(?::' + IPV6SEG + r'){0,4}%[0-9a-zA-Z]{1,}',       # fe80::7:8%eth0     fe80::7:8%1  (link-local IPv6 addresses with zone ID)
...
@click.option(
    "--ipv4-min-pref", type=int, show_default=True, default=32,
    help='minimum IPv4 prefix (max. subnet)'
)
@click.option(
    "--ipv6-min-pref", type=int, show_default=True, default=48,
    help='minimum IPv6 prefix (max. subnet)'
)
@click.option(
    "--re-substring", type=str, default=None,
    help='regular expression to parse only a substring from incomming line'
)
@click.option(
    "--ignore-zone-id", is_flag=True, default=True,
    help="ignore link-local IPv6 addresses with zone ID"
)
@click.option(
    "--merge", is_flag=True, default=True,
    help="merge IPs and subnets to smallest possible list of CIDR subnets"
)
...
def filter_networks(opts: IPListOptions, stream: IO):
    ...
    for line in stream:
        ...
        for ipvx, ipvx_min_pref in [
                (opts.ipv4, opts.ipv4_min_pref),
                (opts.ipv4, opts.ipv4_min_pref)
        ]:
            ip_cidr_set = set()
